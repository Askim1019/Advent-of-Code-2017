passcode_list = "kvvfl kvvfl olud wjqsqa olud frc
...
def remaining_passcode_count(passcode_list)
	new_array = []
	
	passcode_list = passcode_list.split("\n")
	passcode_list.each do |line|
	  subarray = line.split(" ")
	...
	new_array.each do |newline|
	  modified_array = []
	  
	  newline.each do |element|
	    new_subarray = element.split("")
	    new_subarray.sort!
	    modified = new_subarray.join("")
	    modified_array.push(modified)
	  end
	  
	  final_array.push(modified_array)
	end
	...
	final_array.each do |item|
	  if item.length == item.uniq.length
	    count += 1 
	  else
	    count += 0 
	  end
...
remaining_passcode_count(passcode_list)
